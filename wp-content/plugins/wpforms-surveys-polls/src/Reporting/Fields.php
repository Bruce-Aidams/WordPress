...
class Fields {
	...
	public static function get_survey_field_data( $field, $form_id, $entry_count = 0, $form_data = array() ) {
		...
		// Ignore `abandoned` entries.
		$abandoned = (int) wpforms()->entry->get_entries(
			array(
				'form_id' => $form_id,
				'status'  => 'abandoned',
			),
		...
		$entry_count = ! empty( $entry_count ) ? $entry_count - $abandoned : 0;
	...
	/**
	 * Determite if it's the field with multiple feature.
	 *
	 * @since 1.6.3
	 *
