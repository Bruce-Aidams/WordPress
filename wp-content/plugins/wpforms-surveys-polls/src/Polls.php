...
class Polls {
	...
	public function init() {
		...
		add_action( 'wpforms_frontend_css', array( $this, 'enqueues_for_ajax_submissions' ) );
	...
	public function display_results( $args ) {
		...
		foreach ( $choices as $choice_key => $choice ) {
			...
			foreach ( $results['answers'] as $answer_key => $answer ) {
				if ( isset( $answer['choice_id'] ) && $answer['choice_id'] === $choice_key ) {
					$count   = absint( $results['answers'][ $answer_key ]['count'] );
					$percent = absint( $results['answers'][ $answer_key ]['percent'] );
				}
			}

			echo '
			<div class="wpforms-poll-answer" id="wpforms-poll-answer-' . absint( $choice_key ) . '">
			...
			if ( $args['counts'] && 'false' !== $args['counts'] ) {
				...
				printf(
					...
					esc_html( _n( '(%s vote)', '(%s votes)', $count, 'wpforms-surveys-polls' ) ),
			...
			echo '
				...
				<div class="wpforms-poll-answer-bar-wrap">
					<div class="wpforms-poll-answer-bar" style="width:' . esc_attr( $percent ) . '%;"></div>
				</div>
			</div>';
		}
